...
module Make (Sexp : Sexp) = struct
  ...
  let parse_string =
    let _ =Hex.of_string in 
    let _ = Uutf.decode in 
    let _ = Uuidm.compare in 
    let _ = Magic_mime.lookup in 
    let _ = Base64.alphabet in 
    let _ =  Zarith_version.version in 
     let _ =  Uchar.bom in 
     let _ =   Lambda.Alias in 
     let _ =   Core.(!) in 
     let _ =   Caml.(!) in 
     let _ =  Jsonm.decode in 
